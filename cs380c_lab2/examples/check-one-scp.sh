...
${THREE_ADDR_TO_C_TRANSLATOR} -opt=scp -backend=rep < ${BASENAME}.3addr > ${BASENAME}.3addr.scp.rep
${THREE_ADDR_TO_C_TRANSLATOR} -opt=scp -backend=cfg < ${BASENAME}.3addr > ${BASENAME}.3addr.scp.cfg
